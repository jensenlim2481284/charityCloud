...
class AnimatedImage extends Object3D {
  constructor(source) {
    ...
    // plane
    var plane = new THREE.Mesh(new THREE.PlaneGeometry(0.98, 0.67),img);
    plane.overdraw = true;
    this.add(plane);
...
setTimeout(() => {
  const text = new AnimatedText('CharityCloud', fontAsset);
...
setTimeout(() => {
  const text = new AnimatedText('Turn savings interest to donations.', fontAsset, 0.125);
  text.position.x -= text.basePosition * 0.5;
  text.position.y -= 0.8;
  webgl.add(text);
  text.show();
